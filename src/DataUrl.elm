...
{-| A standard way to construct `DataUrl` value.
...
{-| Yet another way to construct `DataUrl` value.
It is much faster than `fromString`, but it does not validate `data` part.
...
fromStringFuzzy =
    Result.toMaybe << Parser.run DataUrl.Parser.dataUrlFuzzy
